...
public class DepartmentRequestPayout extends GenericForwardComposer{
        ...
        try{
        	...
        	if(S_DocNo == null) {
        		...
        		if(S_IsStatus.equals("0")) {
        			Messagebox.show("ไม่สามารถนำรายการเข้าได้ เนื่องจากเอกสารนี้ส่งไปขอเบิกแล้ว !!", "CSSD", Messagebox.OK, Messagebox.EXCLAMATION);
        			return;
        		}
        	}
	...
	private Listcell newListcell(final boolean IsSave, final String S_DocNo, final String IsStatus){
		...
		if(IsStatus.equals("0")) {
			...
			switch (IsStatus) {
				case "1": S_StatusName = "ค้างจ่าย"; break;
				case "2": S_StatusName = "ปิดเอกสาร"; break;
				case "3": S_StatusName = "ปิดเอกสาร รับเข้าแผนก"; break;
				default: break;
			}
